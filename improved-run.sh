...
function check_env() {
    ...
    "dev" | "rec")
        log "Info: Environment set to 'dev'"
        echo "dev"
        ;;
    "prd")
        # Check if the home directory exists before trying to cd into it.
        if [[ ! -d "$home_directory" ]]; then
            log "Error: Directory '$home_directory' does not exist"
            return 1
        fi

        cd "$home_directory" || return 1
        log "Info: Changed directory to '$home_directory'"
        echo "prod"
        ;;
    *)
        log "Error: environment '$1' does not contain 'dev' or 'prd'"
        return 2
        ;;
    esac
}
...
function check_job() {
    ...
    "main" | "bfi")
        ...
        ;;
    "ods")
        log "Info: Job set to 'ods.py'"
        echo "ods.py"
        ;;
    *)
        log "Error: job does not contain '$job_name'"
        return 1
        ;;
    esac
}
...
function tar_gz_src() {
    ...
    tar_error=$(find "$src_directory" -name "*.py" -printf '%P\n' | tar -czf "$archive_name" -C "$src_directory" -T - 2>&1)
...
function main() {
    ...
    partition_date=$(check_date "$3")
    check_status $? "check_date"

    tar_gz_src "$environment"
    ...
    run_spark_submit "$job_name" "$partition_date" "$temp_log_file"
