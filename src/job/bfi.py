...
def main() -> None:
    ...
    capjour_param2 = load_latest_date_from_table(
        f"{ods_path}/CAPJOUR_PARAM2_OE", shuffle_partitions
    )
    ...
    nouveau_mapping_type_emploi = load_latest_date_from_table(
        f"{from_files_path}/TYPE_EMPLOI", shuffle_partitions
    )
    ...
    objectif_bfi_corpo = load_latest_date_from_table(
        f"{from_files_path}/OBJECTIF_BFI_CORPO", shuffle_partitions
    )
    ...
    ancien_mapping_type_ressource_emploi = (
        build_ancien_mapping_type_ressource_emploi(capjour_param2)
    )
    ...
    taux_change_virement_bam = build_taux_change_virement_bam(virement_bam)

    to_iceberg(
        taux_change_virement_bam,
        "SOCLE",
        "COMPTABILITE",
        "TAUX_CHANGE_VIREMENT_BAM",
        partition_date,
    ...
    taux_change_bbe_bam = build_taux_change_bbe_bam(bbe_bam)
