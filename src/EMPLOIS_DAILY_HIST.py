...
spark = SparkSession.builder.appName("EMPLOIS DAILY HIST").getOrCreate()
...
df_c = spark.read.format("parquet").load("hdfs://10.9.125.134:8020/nifi/DLKPROD/MAPPING_TYPE_EMPLOI/MAPPING_TYPE_EMPLOI")


# Perform the joins as specified in the SQ query
...
result_df = df_a.alias("a") \
    .join(df_c.alias("c"),
          ...
          "left_outer") \
    .join(df_e.alias("e"),
          expr("a.PCI = concat(e.PCI, '0') AND a.NATURE_EXTERNE = e.NATURE AND e.categorie = 'EMP'"),
          "left_outer") \
    .select(col("a.DTTRTM"), col("a.PCI"), col("a.NATURE_EXTERNE").alias("NATURE"), col("a.numerodecompte"), col("a.devise")
            ...
            col("e.Type_RESS_EMPO").alias("ANCIEN_TYPE_EMPLOI")).filter(df_a["soldejourdb"].isNotNull())

# Definition of the domain and table for saving the result-----------------6 530,154
COUCHE = "SOCLE"
DOMAINE = "CDM_COMPTABILITE"
TABLE = "EMPLOIS_DAILY_HIST"
